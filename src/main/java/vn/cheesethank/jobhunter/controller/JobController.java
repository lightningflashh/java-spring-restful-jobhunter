...
import vn.cheesethank.jobhunter.domain.response.ResCreateJobDTO;
import vn.cheesethank.jobhunter.domain.response.ResUpdateJobDTO;
import vn.cheesethank.jobhunter.domain.response.ResultPaginationDTO;
import vn.cheesethank.jobhunter.service.JobService;
...
public class JobController {
    ...
    public ResponseEntity<ResCreateJobDTO> create(@Valid @RequestBody Job job) throws IdInvalidException {
        if (job.getName() == null || job.getName().isEmpty()) {
            throw new IdInvalidException("Job name is required");
        }
        if (job.getLocation() == null || job.getLocation().isEmpty()) {
            throw new IdInvalidException("Job location is required");
        }
